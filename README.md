...
3. **Отредактируйте `.env`** и укажите правильные параметры:
```env
DB_HOST=localhost
DB_PORT=5432
DB_USER=postgres                    # ваш пользователь PostgreSQL
DB_PASSWORD=ваш_реальный_пароль    # ⚠️ ВАЖНО: укажите правильный пароль!
DB_NAME=gambling
DB_SSLMODE=disable
APP_ENV=local
APP_URL=localhost
...
```bash
# Проверьте, что можете подключиться к PostgreSQL
psql -U postgres -d gambling -h localhost
```
...
**Важно:** Если вы видите ошибку о недостающих параметрах БД, убедитесь, что:
- Файл `.env` создан в корне проекта
- Все параметры БД заполнены корректно
