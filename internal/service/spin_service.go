...
var (
	ErrInvalidBetAmount = errors.New("неверная сумма ставки")
)

// SpinService предоставляет методы для игры на спинах
...
// NewSpinService создает новый экземпляр SpinService
func NewSpinService(balanceService *BalanceService, spinRepo *repository.SpinRepository) *SpinService {
	return &SpinService{
		balanceService: balanceService,
		spinRepo:       spinRepo,
...
// SpinResult представляет результат одного спина
...
// Spin выполняет один спин игры
// Вероятности основаны на классических слотах казино с RTP (Return to Player) ~95%
func (s *SpinService) Spin(userID uint, betAmount float64) (*SpinResult, error) {
	if betAmount <= 0 {
		return nil, ErrInvalidBetAmount
	}

	// Проверяем и списываем ставку
	user, err := s.balanceService.Withdraw(userID, betAmount)
	if err != nil {
		return nil, err
	}

	// Генерируем символы на барабанах (0-9, где 0 - самый редкий и ценный)
	reel1 := s.generateSymbol()
	reel2 := s.generateSymbol()
	reel3 := s.generateSymbol()

	// Определяем выигрыш
	winAmount := s.calculateWin(reel1, reel2, reel3, betAmount)
	isWin := winAmount > 0

	// Если есть выигрыш, добавляем его на баланс
	if isWin {
		user, err = s.balanceService.AddWin(userID, winAmount)
		if err != nil {
			return nil, err
		}
	}
	...
	if err := s.spinRepo.Create(spinResult); err != nil {
		return nil, err
	}
...
// Вероятности настроены так, чтобы обеспечить RTP ~95%
// Символы: 0 (самый редкий), 1-9 (обычные)
func (s *SpinService) generateSymbol() int {
...
func (s *SpinService) calculateWin(reel1, reel2, reel3 int, betAmount float64) float64 {
